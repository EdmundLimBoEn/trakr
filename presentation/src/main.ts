...
$('#deck').innerHTML=slides.map((s,i)=>`<section class="slide ${s.layout || 'content-slide'}" id="slide-${i+1}" aria-label="${s.section}" ${i?'hidden':''}>${s.html}</section>`).join('');
...
const traceSteps = ['trace-scan', 'trace-stage', 'trace-commit', 'trace-receipt'];
function setFlow(step:number){
  traceSteps.forEach((id,i)=>$('#'+id).classList.toggle('traced',i<=step));
  $('#trace-status').textContent = ['The tag identifies the equipment','The student records items and condition','Access checks allow the checkout to be saved','The teacher views and updates the shared record'][step] || 'READY / trace the handover';
}
function go(next:number){index=Math.max(0,Math.min(slides.length-1,next));stopFlow();document.querySelectorAll<HTMLElement>('.slide').forEach((el,i)=>{el.hidden=i!==index;el.classList.toggle('active',i===index);el.inert=i!==index;});if(index===3)setFlow(-1);$('#counter').textContent=`${String(index+1).padStart(2,'0')} / ${slides.length}`;$('#section-name').textContent=slides[index].section;$<HTMLButtonElement>('#previous').disabled=index===0;$<HTMLButtonElement>('#next').disabled=index===slides.length-1;$('#progress-fill').style.width=`${(index+1)/slides.length*100}%`;history.replaceState(null,'',`#${index+1}`);document.title=`Trakr · ${slides[index].title}`;window.scrollTo({top:0,behavior:'instant'});updateNotes();}
...
$('#slide-list').innerHTML=slides.map((s,i)=>`<button data-goto="${i}"><span class="mono">${String(i+1).padStart(2,'0')}</span><span>${escape(s.title)}</span></button>`).join('');
