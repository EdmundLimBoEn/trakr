...
final class GearGuardTests: XCTestCase {
    ...
    func testReplacingTagPreservesClaimHistory() throws {
        ...
        XCTAssertThrowsError(try store.resolve(tagID: oldTag)) {
            XCTAssertEqual($0 as? GearGuardError, .inactiveTag)
        }
    ...
    func testSnapshotRoundTripPreservesNewCollections() throws {
        ...
        let encoded = try JSONEncoder().encode(StoreSnapshot(issues: [issue], notifications: [notification], inactiveTagIDs: ["gg:01J9Z6M4Y7X3N8K2D5P0Q1R4TC"]))
        ...
        XCTAssertEqual(decoded.inactiveTagIDs, ["gg:01J9Z6M4Y7X3N8K2D5P0Q1R4TC"])
    }
}
