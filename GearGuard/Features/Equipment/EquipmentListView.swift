        ...
        NavigationStack {
            ...
            .toolbar {
                ...
                ToolbarItem(placement: .confirmationAction) {
                    Button("Save") {
                        do {
                            try store.updateEquipment(id: equipment.id, name: name, serial: serial, isActive: isActive)
                            dismiss()
                        } catch {
                            self.error = error
                        }
...
private struct TagReplacementView: View {
            ...
            Form {
                ...
                Section {
                    ...
                    Button("Use demo replacement") {
                        replace(UID: "SIMULATOR-\(Int.random(in: 1000...9999))")
                    }
    ...
    private func replace(UID: String) {
        do {
            try store.replaceTag(equipmentID: equipment.id, tagID: tagID, hardwareUID: UID)
            dismiss()
        } catch {
            self.error = error
        }
