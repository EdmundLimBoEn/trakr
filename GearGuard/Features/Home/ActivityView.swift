...
struct TeacherDashboardView: View {
    ...
    var body: some View {
        NavigationStack {
                    ...
                    if unresolved.isEmpty {
                        ...
                        ForEach(unresolved) { issue in
                            IssueRow(issue: issue) { status in
                                do { try store.updateIssue(id: issue.id, status: status) }
            ...
            .refreshable { store.processOverdue() }
            .errorAlert($error)
        }
    }
        ...
        VStack(alignment: .leading, spacing: 8) {
            HStack {
                ...
                Menu {
                    ...
                    StatusPill(text: issue.status.title, color: issue.status == .open ? .orange : GearTheme.forest)
                }
            }
            Text(issue.text)
                .font(.subheadline)
...
private struct NotificationRow: View {
    ...
    private var color: Color {
        switch notification.kind {
        case .checkout, .returned: GearTheme.forest
