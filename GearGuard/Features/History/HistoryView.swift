                    ...
                    List(store.claims(for: user)) { claim in
                        HStack(alignment: .top, spacing: 12) {
                            Image(systemName: claim.status == .active ? "arrow.up.right.circle.fill" : "checkmark.circle.fill")
                                .font(.title2)
                                .foregroundStyle(claim.status == .active ? GearTheme.amber : GearTheme.forest)
                            ...
                            Spacer()
                            StatusPill(text: claim.status == .active ? "Active" : "Returned", color: claim.status == .active ? GearTheme.amber : GearTheme.forest)
                        }
                        .padding(.vertical, 5)
                    }
                }
